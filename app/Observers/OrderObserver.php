...
class OrderObserver
{
    ...
    public function updated(Order $order)
    {
        if($order->isDirty('status_livraison_id')){
            ...
            if($order->getOriginal('status_livraison_id') == 2 && $order->status_livraison_id != 1 && $order->status_livraison_id != 6 ){
                $produit = $order->product;
                $produit->quantity += $order->quantity;
                $produit->save();
            }

            if($order->getOriginal('status_livraison_id') == 6 && $order->status_livraison_id != 1 && $order->status_livraison_id != 1 ){
                $produit = $order->product;
                $produit->quantity += $order->quantity;
                $produit->save();
            }
